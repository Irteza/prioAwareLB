...
#### SMI 24-oct-16
set partial_failure [lindex $argv 38]
set failed_tor [lindex $argv 39]
...
puts "partial failure $partial_failure"
...
if {$enableMultiPath == 1} {
    ...
    Classifier/MultiPath set partialFailure_ $partial_failure
...
for {set i 0} {$i < $topology_tors} {incr i} {
    ...
    if { $i==0 } {
	puts "SMI-TEST: Node ID of leftmost ToR = [$n($i) id]"
    }
}
...
## ROUGH SMI -- PARTIAL FAILURE CASE
if { $partial_failure==1 } {
    $ns at 1.01 "$ns bandwidth $n(0) $a(0) [expr $UCap/10]Gb 'duplex'" ; ##$ns bandwidth $n0 $n2 0.1Mb 'duplex'
}
